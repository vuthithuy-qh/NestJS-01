...
export class CatSpecService {
  ...
  async create(createDto: CreateCatSpecDto): Promise<CatSpec> {
    ...
    if (variantOptionIds && variantOptionIds.length > 0) {
      const options =
        await this.variantOptionRepository.findByIds(variantOptionIds);
  ...
  async uploadImage(specId: number, file: Express.Multer.File) {
    const spec = await this.catSpecRepository.findOneBy({ id: specId });
    if (!spec) {
      throw new NotFoundException(`Không tìm thấy CatSpec với id ${specId}`);
    }

    // Xóa ảnh cũ trên Cloudinary (nếu có)
    if (spec.catImage) {
      const publicId = this.cloudinaryService.extractPublicId(spec.catImage);
      if (publicId) {
    ...
    // Lưu URL vào DB
    spec.catImage = result.secure_url;
    await this.catSpecRepository.save(spec);

    return {
      message: 'Upload ảnh CatSpec thành công!',
      imageUrl: result.secure_url,
    };
  }

  private async createConfigurations(
    catSpecId: number,
